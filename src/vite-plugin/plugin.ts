...
export function motionCanvasNarratorPlugin(
  options: MotionCanvasNarratorPluginOptions = {},
): Plugin {
  const {
    audioPath = 'narrator-cache',
  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      server.ws.on(
        ...
        async (message: any, client: any) => {
          try {
            ...
            console.log(
              `Audio file cached: ${filePath} (${fileSizeMB.toFixed(2)}MB)`,
            );
          } catch (error) {
      ...
      server.ws.on('narrator:check-available', (message: any, client: any) => {
        client.send('narrator:available', {});
      });

      // WebSocket handler for checking if audio exists
      server.ws.on(
        'narrator:check-audio',
        async (message: any, client: any) => {
          try {
...
function writeAudioFile(filePath: string, buffer: Buffer): Promise<void> {
